...
> 当前脚本版本：`v0.1.2`。项目需要 root 权限，会修改服务、证书、Nginx、防火墙和部分内核网络参数。请先了解变更范围，并仅用于合法、合规的网络用途。
...
`v0.1.2` 已通过 Sing-box `v1.13.12` 和 `v1.14.0` 的真实内核配置校验。脚本继续兼容旧 VPS 上的 Sing-box `v1.11`，新安装和“更新到最新版”会使用官方最新稳定版本。核心更新采用暂存下载、SHA-256 校验、候选配置预检、原子切换、服务与监听端口验证；失败时自动回滚旧核心和旧配置。
